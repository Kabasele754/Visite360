...
from apps.tours.models import Tour, Scene360, TourUniqueView, TourShare
from .serializers import TourCardSerializer, TourDetailSerializer
...
class PublicTourQueryMixin:
    def get_queryset(self):
        qs = (
            ...
            .annotate(
                scene_count=Count("scenes", filter=Q(scenes__status=Scene360.Status.PUBLISHED, scenes__is_public=True), distinct=True),
        ...
        if q:
            qs = qs.filter(Q(title__icontains=q) | Q(description__icontains=q) | Q(place__name__icontains=q) | Q(place__city__icontains=q) | Q(place__country__icontains=q))
        if category:
...
class PublicTourDetailAPIView(PublicTourQueryMixin, RetrieveAPIView):
    ...
    def get_object(self):
        return get_object_or_404(self.get_queryset(), organization__slug=self.kwargs["organization_slug"], id=self.kwargs["tour_id"])
...
class PublicTourHeroAPIView(PublicTourQueryMixin, APIView):
    ...
    def get(self, request, *args, **kwargs):
        tour = self.get_queryset().filter(is_featured=True).first() or self.get_queryset().first()
        if not tour:
            return Response({"detail": "No published tour found."}, status=status.HTTP_404_NOT_FOUND)
...
class PublicTourEngagementAPIView(APIView):
    ...
    def post(self, request, organization_slug, tour_id, *args, **kwargs):
        ...
        with transaction.atomic():
            if action == "view":
                _, created = TourUniqueView.objects.get_or_create(
                    ...
                    defaults={"user": user, "ip_hash": ip_hash, "user_agent_hash": ua_hash},
                )
                if created:
                ...
                TourShare.objects.create(tour=tour, user=user, visitor_key=visitor_key, channel=channel, ip_hash=ip_hash, user_agent_hash=ua_hash)
            else:
                return Response({"detail": "Invalid action. Use view or share."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if new_key:
            response.set_cookie("vtour_visitor_id", visitor_key, max_age=60 * 60 * 24 * 365, httponly=True, samesite="Lax")
        return response
