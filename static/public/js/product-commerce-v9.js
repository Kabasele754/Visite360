(function(){"use strict";window.TWINSCOPE_COMMERCE_UI_VERSION="stable-commerce-v9-20260717";
...
async function updateCart(form,input){
 ...
 const group=form.closest("[data-vendor-cart-group]");
 ...
 try{
  ...
  if(data.removed){
   ...
   if(lineTotal) lineTotal.textContent=money(data.currency,data.line_total);
   row?.querySelector("[data-cart-minus]")?.toggleAttribute("disabled",Number(data.quantity)<=1);
   row?.querySelector("[data-cart-plus]")?.toggleAttribute("disabled",Number(data.quantity)>=Number(data.max_quantity||99));
  }
  ...
  if(group){
   ...
   group.querySelectorAll("[data-line-total]").forEach(element=>{
    const numeric=Number(String(element.textContent).replace(/[^\d.-]/g,""));
    if(Number.isFinite(numeric))vendorSubtotal+=numeric;
   });
   const vendorSubtotalEl=group.querySelector("[data-vendor-subtotal]");
   if(vendorSubtotalEl)vendorSubtotalEl.textContent=money(data.currency,vendorSubtotal);
  }
  ...
  console.error("CART_UPDATE_FAILED",error);
  const message=document.getElementById("marketCartToast");
  if(window.VirtualTourLoader)window.VirtualTourLoader.hide(0);
  if(!navigator.onLine){
   alert("You are offline. Reconnect to update the cart.");
  }else{
   alert(error.message||"Unable to update the cart.");
  }
 }finally{
...
document.addEventListener("click",event=>{
 ...
 input.value=plus
  ? Math.min(max,Number(input.value||1)+1)
  : Math.max(1,Number(input.value||1)-1);
 updateCart(form,input);
});

document.addEventListener("change",event=>{
 const input=event.target.closest("[data-cart-quantity] input[name='quantity']");
 if(!input)return;
 const form=input.closest("[data-cart-quantity]");
 const max=Number(input.max||99);
